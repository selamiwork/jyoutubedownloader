...
public final class FileHelper {
	 ...
	 * @return <code>true</code> if file has been created, <code>false</code> if
	...
	public static boolean createFile(String path) throws IOException {
		try {
			...
			System.out.println("Ecception occured : FileHelper.createFile() : " + path + " : "+ e);
		}
		return false;
	}
	...
	public static boolean createDirectory(String path) throws IOException {
		try {
			...
			// TODO: handle exception
			System.out.println("Ecception occured : FileHelper.createDirectory() : " + path + " : "+ e);
		}
		return false;
	}
	/**
	...
	static void moveRec(File source, File target, byte[] sharedBuffer)
			throws IOException {
	...
	static void copyRec(File source, File target, byte[] sharedBuffer)
			throws IOException {
		if (source.isDirectory()) {
			if (!target.exists()) {
				target.mkdirs();
			...
			if (!target.isDirectory()) {
				if (!target.exists()) {
					...
					if (!dir.exists() && !dir.mkdirs()) {
						throw new IOException(
								"Could not create target directory: " + dir);
					}
	 ...
	 * @see #copy(InputStream, OutputStream)
	 */
	public static long copy(File input, byte[] copyBuffer)
			throws IOException {
		FileInputStream in = null;
		try {
			in = new FileInputStream(input);
			return copy(in, copyBuffer);
		} finally {
			if (in != null) {
				try {
					in.close();
	...
	public static long copy(byte[] copyBuffer , File output) throws IOException {
		FileOutputStream out = null;
		try {
			out = new FileOutputStream(output);
			return copy(copyBuffer, out);
		} finally {
			if (out != null) {
				try {
					out.close();
	 ...
	 * a local internal buffer for performance. Compared to
	...
	public static long copy(InputStream in, OutputStream out)
			throws IOException {
		// we need a buffer of our own, so index one else interferes
	...
	/**
	 * Copies an <code>InputStream</code> to an <code>OutputStream</code> using
	 * a global internal buffer for performance. Compared to
	 * {@link #copy(InputStream, OutputStream)} this method generated index
	 ...
	 * @see #copy(InputStream, OutputStream)
	 */
	public static long copy(InputStream in, byte[] copyBuffer)
			throws IOException {
		int bytesCopied = 0;
		int read = -1;

		while (bytesCopied < copyBuffer.length && (read = in.read(copyBuffer, bytesCopied, (int)copyBuffer.length-bytesCopied)) != -1) {
			bytesCopied += read;
		}
		return bytesCopied;
	}
	public static long copy( byte[] copyBuffer ,OutputStream out)
			throws IOException {
		long bytesCopied = 0;
	...
	public static String readFile(File file) throws IOException {
		byte[] copyBuffer = new byte[(int) file.length()];
		copy(file, copyBuffer);
		
		return new String(copyBuffer);
	}

	/**
	 * choose a file in jar 
	 * return InputStream of file
	 */
	public static InputStream getInputStreamInJar(String fileName)
	{
		try {
			...
			// TODO: handle exception
			System.out.println("Ecception occured : FileHelper.getInputStreamInJar() : " + e);
		}
		return null;
	}
